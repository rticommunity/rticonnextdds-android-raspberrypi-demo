...
static int publisher_main(int domainId, int sample_count) {
	...
	MotorControlDataWriter *MotorControl_writer = NULL;
	MotorControl *instance = NULL;
	DDS_ReturnCode_t retcode;
	...
	MotorControl_writer = MotorControlDataWriter_narrow(writer);
	if (MotorControl_writer == NULL) {
	...
	instance = MotorControlTypeSupport_create_data_ex(DDS_BOOLEAN_TRUE);
	if (instance == NULL) {
		printf("MotorControlTypeSupport_create_data error\n");
		publisher_shutdown(participant);
		return -1;
	}
	...
	/*
	 retcode = MotorControlDataWriter_unregister_instance(
	 MotorControl_writer, instance, &instance_handle);
	 if (retcode != DDS_RETCODE_OK) {
	...
	/* Delete data sample */
	retcode = MotorControlTypeSupport_delete_data_ex(instance,
			DDS_BOOLEAN_TRUE);
	if (retcode != DDS_RETCODE_OK) {
		printf("MotorControlTypeSupport_delete_data error %d\n", retcode);
	}
