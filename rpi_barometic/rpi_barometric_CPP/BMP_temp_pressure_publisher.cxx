...
extern "C" int publisher_main(int domainId, int sample_count) {
	...
	BMP_pressureDataWriter * BMP_pressure_writer = NULL;
	BMP_pressure *instance = NULL;
	DDS_ReturnCode_t retcode;
	DDS_InstanceHandle_t instance_handle = DDS_HANDLE_NIL;
	const char *type_name = NULL;
	int count = 0;

	DDS_Duration_t send_period = { 0, 10000000 }; //10ms
	...
	type_name = BMP_pressureTypeSupport::get_type_name();
	retcode = BMP_pressureTypeSupport::register_type(participant, type_name);
	if (retcode != DDS_RETCODE_OK) {
	...
	/* To customize topic QoS, use
	 the configuration file USER_QOS_PROFILES.xml */
	topic = participant->create_topic("Example BMP_pressure", type_name,
	...
	BMP_pressure_writer = BMP_pressureDataWriter::narrow(writer);
	if (BMP_pressure_writer == NULL) {
	...
	instance = BMP_pressureTypeSupport::create_data();

	if (instance == NULL) {
		printf("BMP_pressureTypeSupport::create_data error\n");
		publisher_shutdown(participant);
		return -1;
	}
	...
	/*
	 instance_handle = BMP_pressure_writer->register_instance(*instance);
	 */
	...
	for (count = 0; (sample_count == 0) || (count < sample_count); ++count) {
		...
		//  printf("Writing BMP_pressure, count %d\n", count);
		...
		try {
			...
			instance->id = "RTI UK";
			instance->Pressure = data.kPa;
			instance->Altitude = BMP085::getRelativeAltitude(data.kPa);
			instance->Temperature = data.celcius;

		} catch (BMP085::smbusIOException &ex) {
			cerr << "SMBus I/O Error:\n" << ex.what();
		}
		retcode = BMP_pressure_writer->write(*instance, instance_handle);
		if (retcode != DDS_RETCODE_OK) {
	...
	/*
	 retcode = BMP_pressure_writer->unregister_instance(
	...
	/* Delete data sample */
	retcode = BMP_pressureTypeSupport::delete_data(instance);
	if (retcode != DDS_RETCODE_OK) {
		printf("BMP_pressureTypeSupport::delete_data error %d\n", retcode);
	}
