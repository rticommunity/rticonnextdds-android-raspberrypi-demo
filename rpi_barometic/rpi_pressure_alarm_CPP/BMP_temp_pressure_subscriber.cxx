...
{
	//count_th=1;
  //system("aplay ~/sound_wav/IndustrialAlarm.wav");
 system("aplay ~/sound_wav/disco_5.wav");
//	printf("\a");
    // long tid;
     //tid = (long)threadid;
    // printf("Amarm Sound! Thread ID=%d ",  tid );
     pthread_exit(NULL);
}
...
{/*
    ...
    BMP_pressure_reader = BMP_pressureDataReader::narrow(reader);
    if (BMP_pressure_reader == NULL) {
        printf("DataReader narrow error\n");
        return;
    }

    retcode = BMP_pressure_reader->take(
   ...
   // for (i = 0; i < data_seq.length(); ++i) {
    int i=1;
        ...
        if (info_seq[i].valid_data) {
              // BMP_pressureTypeSupport::print_data(&data_seq[i]);

        	if((data_seq[i].Pressure>=mPressure_high)|| (data_seq[i].Pressure<=mPressure_low))
        	     			{
...
//----xxxxxxxxxxxxxxxxxxxxxxxx END xxxxxxxxxxxxxxxx
...
void BMP_pressure_rangeListener::on_data_available(DDSDataReader* reader)
{
    ...
    retcode = BMP_pressure_range_reader->take(
        data_seq, info_seq, DDS_LENGTH_UNLIMITED,
        DDS_ANY_SAMPLE_STATE, DDS_ANY_VIEW_STATE, DDS_ANY_INSTANCE_STATE);
    ...
    for (i = 0; i < data_seq.length(); ++i) {
        if (info_seq[i].valid_data) {
           //BMP_pressure_rangeTypeSupport::print_data(&data_seq[i]);
        	//mid1 = data_seq[i].id1;
   ...
   // -----Example BMP_pressure----
    /* To customize the subscriber QoS, use
       ...
       topic2 = participant->create_topic(
           "Example BMP_pressure",
           type_name2, DDS_TOPIC_QOS_DEFAULT, NULL /* listener */,
           DDS_STATUS_MASK_NONE);
       if (topic2 == NULL) {
           printf("create_topic error\n");
           subscriber_shutdown(participant);
      ...
      // reader_listener2 = new BMP_pressureListener();
       ...
       /* To customize the data reader QoS, use
          the configuration file USER_QOS_PROFILES.xml */
       reader2 = subscriber2->create_datareader(
...
bool flag_set=false;
  ...
  //  for (count=0; (sample_count == 0) || (count < sample_count); ++count) {
      ...
      //  printf("BMP_pressure_range subscriber sleeping for %d sec...\n",
            //   receive_period.sec);
