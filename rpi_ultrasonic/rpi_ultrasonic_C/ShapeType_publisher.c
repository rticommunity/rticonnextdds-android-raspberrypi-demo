...
static int publisher_main(int domainId, int sample_count) {
	...
	DDS_Topic *topic_square = NULL;

	DDS_DataWriter *writer_square = NULL;

	ShapeTypeDataWriter *ShapeType_writer_square = NULL;
	...
	/* To customize topic QoS, use
	 the configuration file USER_QOS_PROFILES.xml */
	topic_square = DDS_DomainParticipant_create_topic(participant, "Square",
	...
	/* To customize data writer QoS, use
	 the configuration file USER_QOS_PROFILES.xml */
	writer_square = DDS_Publisher_create_datawriter(publisher, topic_square,
			&DDS_DATAWRITER_QOS_DEFAULT, NULL /* listener */,
			DDS_STATUS_MASK_NONE);
	if (writer_square == NULL) {
		printf("create_datawriter error\n");
		publisher_shutdown(participant);
		return -1;
	}

	ShapeType_writer_square = ShapeTypeDataWriter_narrow(writer_square);
	if (ShapeType_writer_square == NULL) {
		printf("DataWriter_square narrow error\n");
		publisher_shutdown(participant);
		return -1;
	}
	...
	if (instance == NULL) {
		printf("ShapeTypeTypeSupport_create_data error\n");
		publisher_shutdown(participant);
		return -1;
	}
	...
	while (1) {
		...
		/* Write data */
		retcode = ShapeTypeDataWriter_write(ShapeType_writer_square, instance,
				&instance_handle);
		if (retcode != DDS_RETCODE_OK) {
	...
	if (retcode != DDS_RETCODE_OK) {
		printf("ShapeTypeTypeSupport_delete_data error %d\n", retcode);
	}
